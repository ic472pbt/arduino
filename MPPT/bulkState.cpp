...
IState* bulkState::Handle(Charger& charger, SensorsData& sensor, unsigned long currentTime) 
{
      ...
      if (sensor.PVvoltage + 0.2 < sensor.batteryV) {                        // if watts input from the solar panel is less than          
      ...
      else if (charger.rawBatteryV > sensor.floatVoltageRaw + sensor.tempCompensationRaw - charger.powerCompensation){
        charger.pwmController.storeMpptDuty();
        charger.controlFloat = true;                                // else if the battery voltage has gotten above the float
        newState = &charger.floatInstance;              // battery float voltage go to the charger battery float state            
          ...
          newState = &charger.onInstance;                            // it means there is not much power being generated by the solar panel
          charger.pwmController.setMaxDuty();
        }
          ...
          if(charger.mpptReached == 1 || charger.startTracking){
            if(currentTime - lastTrackingTime > 29900 || charger.startTracking){
              ...
              delta = charger.floatInstance.dirrection * stepSize;
        ...
        /*   Serial.print("power before: "); Serial.print(rawPowerPrev); 
           ...
           if(solarPower > rawPowerPrev){
                ...
                if(delta == 0){                           //  MP MV ; MPP Reached - 
                  ...
                  charger.floatInstance.Reverse();               // Change the direction for the next tracking.
                  charger.mpptReached = 1; // ! indicate MPPT reached
                } 
              }
          }                    
        }
