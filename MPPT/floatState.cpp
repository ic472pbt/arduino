...
IState* floatState::Handle(Charger& charger, SensorsData& sensor, unsigned long currentTime) 
{
          ...
          int effectiveBound = sensor.floatVoltageRaw + charger.tempCompensationRaw - charger.powerCompensation;
          charger.mpptReached = 0;
          if(!charger.finishEqualize && sensor.rawBatteryV > sensor.floatVoltageRaw + charger.tempCompensationRaw - 25){
          ...
          if (sensor.PVvoltage + 0.2 < sensor.batteryV) {        // if watts input from the solar panel is less than
            ...
            int delta = (sensor.rawBatteryV - (sensor.floatVoltageRaw + charger.tempCompensationRaw - charger.powerCompensation)) / 2 
              // + charger.stepsDown * 2
              ; 
            ...
            if (sensor.rawBatteryV < sensor.floatVoltageRaw + charger.tempCompensationRaw - 40){ //(floatVoltage + tempCompensation - 0.6)) {               // if the voltage drops because of added load,
