...
IState* offState::Handle(Charger& charger, SensorsData& sensor, unsigned long currentTime) 
{
      ...
      bool solarOff = true;
      int floatV = sensor.floatVoltageRaw + sensor.tempCompensationRaw;
      ...
      else if ((charger.rawBatteryV > floatV) && (sensor.PVvoltage > sensor.batteryV)) {
          newState = &charger.floatInstance;                         // if battery voltage is still high and solar volts are high
          solarOff = false;
      }    
      else if ((sensor.batteryV > LVD) && (charger.rawBatteryV < floatV) && (sensor.PVvoltage > sensor.batteryV)) {
